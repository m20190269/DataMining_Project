...
data=pd.read_csv('.\\Data\\A2Z_Insurance.csv')
...
#counting duplicate rows that we might have
#data.duplicated(subset=None, keep='first').sum()
#we have 3, lets remove them
...
data[['Premiums in LOB: Motor','Premiums in LOB: Household', 'Premiums in LOB: Health',
       'Premiums in LOB:  Life', 'Premiums in LOB: Work Compensations']] = df_temp.copy()
...
temp_data=data.loc[:,['First Policy´s Year', 'Educational Degree', 'Geographic Living Area']].copy()
...
my_data = data.loc[:,['Premiums in LOB: Motor', 'Has Children (Y=1)', 'Gross Monthly Salary']].copy()

#we will separate the Has Children by incomplete info and complete info, to train the model
my_data_incomplete = my_data.loc[my_data.loc[:,'Has Children (Y=1)'].isna()].copy()
my_data_complete = my_data[~my_data.index.isin(my_data_incomplete.index)].copy()
...
del dist, numerical_columns, zeros, count_zeros, numbr_zeros, df_temp, corr1, temp_data, my_data_to_regress
del Education, my_data_to_label_OneHot_s, new_data_regress, my_data_to_reg_incomplete, my_data_to_reg_complete
del imputed_GMS, temp_df, data_join1, my_data, my_data_incomplete, my_data_complete, imputed_child, temp_dff, data_join2
...
del fidelity_time, premiuns_corrected, premiuns_total, anual_salary, salary_prop, premiuns, props_premiuns

del corr, corr2, dist_f, x
...
sil_plot(2, client_norm, np.asarray(client_finalables))

del client1_labels, client_rednorm, client_HC, numclientHC_labels, clust200_labels, client_finalables, numclient_labels
##########################################################################################################################
############################################### KMODES Client #############################################################
...
del lista, clusters, catclient_labels, unique, counts, cat_counts, clusters2, unique2, counts2, cat_counts2
###################################################################################################################
###################################################################################################################
...
sil_plot(5, cons_norm, np.asarray(consum_finalables))
...
sil_plot(4, cons_norm, np.asarray(consum_finalables1))
...
sil_plot(3, cons_norm, np.asarray(new_ConsumLabels))

del my_clusters, consum1_labels, cons_rednorm, consum_HC, consumHC_labels, consum200_labels, consumHC_result_to_revert
del consum_finalables, consum_labels, consum, consum_HC1, consumHC_labels1, consum200_labels1, consumHC_result_to_revert1
del consum_finalables1, consum_labels1, consum1, new_ConsumLabels, new_consum200
#######################################################################################################################
...
sil_plot(2, cons_norm, np.asarray(consum_finalables))
#very balanced
...
sil_plot(3, client_norm, np.asarray(client_finalables))
...
del X, names, som_clusters, som_labels, som_clusters_, som_consum_clusters_torevert, clust100_consum_labels, consum_finalables
del som_client_clusters, som_client_labels, som_client_clusters_torevert, clust100_client_labels, client_finalables
##############################################################################################################################
######################################################  DBSCAN  ##############################################################
##############################################################################################################################
...
del labels, n_clusters_, unique_clusters
...
del to_MS, my_bandwidth, ms, labels, cluster_centers, labels_unique, n_clusters_, ms_client_clusters, unique, counts, ms_consum_clusters
...
del km_cons_clusters, cons_clusters1
...
sil_plot(3,client_norm, client_HC.labels_)
...
sil_plot(2,client_norm, client_HC1.labels_)

del client_HC_labels, client_HC_labels1, HC_client_labels1, HC_client_to_revert1
## CONSUMPTION
...
sil_plot(3,cons_norm, cons_HC.labels_)
...
HC_cons_clusters1 = pd.DataFrame(scaler.inverse_transform(X=HC_cons_to_revert1), 
                                ...
                                'Premiums in LOB: Health', 'Premiums in LOB:  Life',
                                'Premiums in LOB: Work Compensations',])
sil_plot(2,cons_norm, cons_HC1.labels_)
#worst than the first solution
del cons_HC_labels, cons_HC_labels1, HC_cons_labels1, HC_cons_to_revert1
###############################################################################################################################
###############################################################################################################################
...
#keans + HC
total_distance(to_revert, client_kmHC)  
#SOM
total_distance(som_client_torevert, client_som_clusters)
#kmeans
total_distance(km_norm_client_clusters, km_client_labels)  #the best!
#HC
total_distance(HC_client_to_revert, HC_client_labels)
...
#keans + HC
total_distance(consum_torevert2, consum2) #the best!
#SOM
total_distance(som_consum_torevert, som_cons_clusters)
#kmeans
total_distance(km_norm_cons_clusters, km_cons_labels) 
#HC
total_distance(HC_cons_to_revert, HC_cons_labels)
...
del client_good0, consum_good0, client_good1, consum_good1, move00_01, move00_02, move00_20, move10_11, move10_12, move10_20
...
del Affinity, x, y, x_train, x_test, y_train, y_test, lista1, lista2, outliers1, outliers_labels, outliers_clf
del final_data0, outliers, dot_data
...
del categoricals, i, cl, clusters1, sizes, clusters2, clusters, avg_data, clusters_F_norm, clust, colum
del Z, j, k, labels, new_labels
